...
(x)     - Соотв-т 'x' и запоминает это соотв-е (захват-е скобки). См.ниже
(?:x)   - Соотв-т 'x' но не запоминает соотв-е (не-захват-е скобки).
...
Группировка с помощью кругл.скобок позволяет 
подставлять подходящ. значения в переменные $1, $2, ...

Пример: 
```
RewriteRule ^([a-zA-Z]+)/([0-9]{4})/?test.php?cat=$1&year$2
```
В примере выше: ([a-zA-Z]+) подставл-ся в cat=$1 и т.д.
<br>

```
[\b]    - Backspace (U+0008). (Не путать с \b.)
\b      - Граница слова.
\B      - Несловообразующая граница.
\cX     - Управл. символ в строке. /\cM/ соотв-ет control-M (U+000D) в строке.
